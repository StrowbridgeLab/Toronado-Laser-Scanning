...
last revised 6 Dec 2017 BWS v1.0
...
class clsImageWin(QtGui.QMainWindow):
    ...
    """
    def __init__(self, callingInstance, winNum, useNativeGUI):
    ...
    def displayCursorInfo(self, posIn=None, passDict=None):
        ...
        if posIn:
            ...
            if self.cursorMode:
                ...
                self.mw.setWindowTitle(tempStr)
    ...
    def changeZoom(self, newDisplayZoom):
        ...
        if newDisplayZoom in allowedZooms:
            self.mw.resize(int(newDisplayZoom * self.retDict["Xsize"]), int(newDisplayZoom * self.retDict["Ysize"]))
    ...
    def refreshImageDisplay(self, passDict=None):
        ...
        if self.autoDisplayLevel or not self.maxDisplayLevel:
            self.maxDisplayLevel = passDict["channelMaxValues"][self.curChannel]
        if self.displayColorScheme:
    ...
    def updateROIs(self, roi):
        self.mw.setWindowTitle(str(np.mean(roi.getArrayRegion(self.retDict["data"][0][0], self.curImage))))
    ...
    def initUI(self, winNum):
        ...
        self.lastPos = None
        self.view = self.mw.addViewBox(enableMouse=False)
        self.view.setCursor(QtCore.Qt.CrossCursor)
        ...
        self.rois = []
        self.minDisplayLevel = 0
        self.maxDisplayLevel = None
        self.autoDisplayLevel = True
        self.displayzoom = 0 # changed 25 May 2016 now need to check on refresh
        self.displayColorScheme = "w r"
        self.colorizeDepth = False
        self.filterString = ""
        self.saturationPercentage = 95.
        self.mw.setGeometry(490, 200, 200, 200)
        self.mw.show()
